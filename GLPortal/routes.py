...
from models import (
    ...
    delete_user, insert_dummy_data, delete_all_for_parent, update_incident_status
)
from database import get_connection

routes_bp = Blueprint("routes_bp", __name__)

ADMIN_SECRET_KEY = "supersecretadmin123"
...
def child_dashboard():
    ...
    for inc in incidents:
        evidence = get_incident_evidence(inc["incident_id"])
        incidents_with_evidence.append({"incident": inc, "evidence": evidence})

    return render_template("child_dashboard.html",
                           incidents=incidents,
                           incidents_with_evidence=incidents_with_evidence,
                           accounts=accounts)
...
def add_incident_route():
    ...
    add_incident(session["user_id"], platform, sender_handle,
                 incident_type, message_text, severity)
...
@routes_bp.route("/update-status/<int:incident_id>", methods=["POST"])
def update_status(incident_id):
    ...
    flash(f"Incident marked as {new_status}.", "success")
...
@routes_bp.route("/delete-my-account", methods=["GET", "POST"])
def delete_my_account():
    ...
    if request.method == "POST":
        ...
        if not user:
            ...
            return render_template("delete_my_account.html")
...
@routes_bp.route("/admin/purge-all", methods=["GET", "POST"])
def admin_purge_all():
    if request.method == "POST":
        key = request.form.get("admin_key", "")
        if key != ADMIN_SECRET_KEY:
            flash("Unauthorized. Wrong admin key.", "danger")
        ...
        conn.execute("DELETE FROM child_links")
        conn.execute("DELETE FROM users")
        conn.commit()
        conn.close()
        session.clear()
        flash("All users and data have been purged.", "success")
        return redirect(url_for("routes_bp.register"))
